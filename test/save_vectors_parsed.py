...
def getOutWordBatches(data_json,out_names,batch_size):
	out_data = []
	for data in data_json:
		out_data.append(out_names.index(data['def_word']))

	data_batch = []

	num_batches = math.ceil(len(out_data)/batch_size)
	for batch_num in range(num_batches):
		start = batch_num*batch_size
		end = min(start+batch_size,len(data_json))

		cur_batch = out_data[start:end]
		data_batch.append(cur_batch)
...
batches = getOutWordBatches(data_json,out_names,85)
...
with tf.Session() as sess:
	...
	out_embedding = np.array([])
	for batch in batches:
		print('cur batch')
		print(batch)
		evaluated_out_embedding = model_recursive_nn.getOutEmbeddings(sess,
			W_out,
			b_out,
			batch)

		if len(out_embedding)==0:
			out_embedding = evaluated_out_embedding
		else:
			out_embedding = np.concatenate((out_embedding,evaluated_out_embedding),axis=0)


with open(config.out_embedding_arr_path_parsed,'w') as f:
	json.dump(out_embedding.tolist(),f)
with open(config.out_names_path_parsed,'w') as f:
	json.dump(out_names,f)
