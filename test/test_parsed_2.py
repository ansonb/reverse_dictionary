...
dictionary, vocabulary_size = data_helpers.loadVocabParsed(dictionary_path=config.dictionaryPathParsed_2)

pos_dict = data_helpers.getPosDict()
...
with tf.Session() as sess:
	...
	for cur_example,data in enumerate(data_json_to_be_modified):
		...
		evaluated_logits, _ = model_recursive_nn_2.run_test_graph(sess,
			W,
			V_in,
			V_out,
			b,
