...
file = './../data/sample/one_word_noise_sample_extra_large.json'
...
with open(file, 'r') as f:
	...
	for def_word,def_arr in data.items():
		...
		for text in line_arr:
			subprocess.call([    
			'echo "' + text + '" | docker run --rm -i brianlow/syntaxnet-docker > syntax_net_output.txt'
			], shell = True)
			

			with open('syntax_net_output.txt','r') as f1:
...
with open('../data/sample/one_word_parsed_sample.json','w') as f:
